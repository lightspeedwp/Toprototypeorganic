...
**Decision rule:** Keep the file in the most logical location. Merge content into the canonical version. Add a deprecation notice to the duplicate.
...
3. If not, rewrite the file using the appropriate template as a base.
...
1. **If content is still valid:** Merge into the canonical file. Update its version and frontmatter.
2. **If content is outdated:** Add a deprecation notice at the top:
...
3. **If content is empty or trivially small:** Delete the file after verifying zero references.
...
2. Update all references in `Guidelines.md`, `_templates.md`, and other files.
3. Verify the Quick Reference table in `Guidelines.md` is complete and accurate.

### Step 6: Save Report

Save the audit report to `/reports/YYYY-MM/guidelines-cleanup-audit.md`.
...
- [ ] Remaining tasks added to `/tasks/task-list.md`
...
- **When rewriting files with templates:** copy the meaningful content from the old file into the template structure. Do not lose any rules or standards.
